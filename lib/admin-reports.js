...
export async function getAdminOrders({ query = "", page = 1, pageSize = 25 } = {}) {
...
export async function getOrderSummary() {
  const { data, error } = await getSupabaseAdmin()
    .from("coupons")
    .select("menu_item_name,qty,price");
  ...
  for (const coupon of data) {
    ...
    current.sold_qty += Number(coupon.qty);
    current.revenue += Number(coupon.qty) * Number(coupon.price);
