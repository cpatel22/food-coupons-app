...
export async function sendNotifications({ email, phone, items, successUrl }) {
    console.log("sendNotifications triggered for:", { email, phone, itemsCount: items.length });

    if (!APP_CONFIG.NOTIFICATIONS.EMAIL_ENABLED && !APP_CONFIG.NOTIFICATIONS.SMS_ENABLED) {
    ...
    if (APP_CONFIG.NOTIFICATIONS.EMAIL_ENABLED && email) {
        try {
            ...
            if (!process.env.SMTP_HOST) {
                console.warn('SMTP_HOST is not defined in .env.local. Skipping email notification.');
            } else {
                ...
                const mailOptions = {
                    from: process.env.EMAIL_FROM || '"My Restaurant" <noreply@example.com>',
                    to: email,
    ...
    if (APP_CONFIG.NOTIFICATIONS.SMS_ENABLED && phone) {
        try {
            console.log(`[SMS NOTIFICATION] To: ${phone} | Content: Your order is confirmed! View your coupons here: ${successUrl}`);
