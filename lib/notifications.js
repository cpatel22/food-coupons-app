...
function getCouponHtml(item) {
  return `
        <div style="font-family: 'Courier New', Courier, monospace; width: 300px; padding: 20px; border: 1px dashed #000; margin: 20px auto; background: #fff; color: #000;">
            <h2 style="text-align: center; margin-bottom: 5px; margin-top: 0;">${APP_CONFIG.BUSINESS_NAME}</h2>
  ...
  if (APP_CONFIG.NOTIFICATIONS.EMAIL_ENABLED && email) {
    try {
      ...
      if (!process.env.SMTP_HOST) {
        ...
        const mailOptions = {
          from:
            process.env.EMAIL_FROM || `"${APP_CONFIG.BUSINESS_NAME}" <noreply@example.com>`,
          to: email,
          subject: `Your Food Coupons - ${APP_CONFIG.BUSINESS_NAME}`,
          html: `
